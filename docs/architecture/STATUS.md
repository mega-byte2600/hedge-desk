...
- `python3 -m unittest discover -s tests -v`: 264 deterministic tests pass
  locally.
...
- The declared synthetic suite contains 63 strategy, timing, execution,
  compliance, and lifecycle war games plus five combined-MVP capital-path
  stresses. Thirty-seven scenarios are explicit `NO_TRADE` controls.
