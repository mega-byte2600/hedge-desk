...
LICENSE_ALLOWLIST = {"mit", "apache-2.0", "bsd-2-clause", "bsd-3-clause", "cc0-1.0", "unlicense"}
...
def _score(*, dataset_signal: bool, open_license: bool, stars: int, asset_classes: tuple[str, ...], regions: tuple[str, ...], bond_priority: bool) -> int:
    score = 35 if dataset_signal else 0
    score += 20 if open_license else 0
    score += 25 if bond_priority else 0
...
def _github_json(url: str, token: str | None = None) -> dict:
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "emporion-data-scout/1.0", "X-GitHub-Api-Version": "2022-11-28"}
    if token:
...
def search_github(query: str, *, token: str | None = None, per_page: int = 20) -> list[DatasetCandidate]:
    payload = _github_json(f"https://api.github.com/search/repositories?q={quote_plus(query)}&sort=updated&order=desc&per_page={per_page}", token)
    ...
    for item in payload.get("items", []):
        ...
        commercial = "OPEN_LICENSE_REVIEW_STILL_REQUIRED" if open_license else ("LICENSE_REVIEW_REQUIRED" if spdx else "NO_LICENSE_METADATA_DO_NOT_USE")
        out.append(DatasetCandidate(
            repo=name, url=item.get("html_url") or "", description=desc, stars=stars,
            updated_at=item.get("updated_at") or "", license_spdx=spdx, open_license=open_license,
            dataset_signal=dataset_signal, commercial_use_status=commercial, discovered_at=now, query=query,
            asset_classes=asset_classes, regions=regions, bond_priority=bond_priority,
            discovery_score=_score(dataset_signal=dataset_signal, open_license=open_license, stars=stars,
                                   asset_classes=asset_classes, regions=regions, bond_priority=bond_priority),
        ))
    return out
...
def discover(queries: Iterable[str] = DEFAULT_QUERIES, *, token: str | None = None) -> dict:
    ...
    ranked = sorted(seen.values(), key=lambda x: (x.bond_priority, x.discovery_score, x.dataset_signal, x.open_license, x.stars, x.updated_at), reverse=True)
    return {
        "schema_version": "emporion-data-scout-1.2.0",
        "purpose": "OPEN_PUBLIC_DATASET_DISCOVERY_ONLY",
        "priority": "BONDS_RATES_CREDIT_FIRST",
        "commercial_use_assumed": False,
        "model_training_enabled": False,
        "ranking_note": "Bond/rates/credit datasets are deliberately prioritized. Discovery is not legal or data-quality approval.",
