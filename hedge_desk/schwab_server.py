"""Dependency-free local API for the paper-only iOS MVP (Schwab read-only).

Moved out of ``server.py`` during a rebase so it stays distinct from the
full-stack web-console server that already lives at ``hedge_desk/server.py``.
"""
